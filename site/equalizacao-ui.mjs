...
render();
fetch('data/portfolios.json').then(r=>{if(!r.ok)throw Error();return r.json()}).then(d=>{const ul=document.createElement('ul');for(const c of d.companies){const li=document.createElement('li'),a=document.createElement('a');a.href='index.html#empresa='+encodeURIComponent(c.id);a.textContent=c.name;li.append(a,document.createTextNode(' — não conciliado'));ul.append(li)}$('portfolio-state').replaceChildren(ul)}).catch(()=>{$('portfolio-state').textContent='Estado indisponível. Consulte o catálogo e o arquivo JSON.'});
